<template>
  <div class="relativew-full left-0 right-0 lg:p-8 p-4 mb-16 bg-home flex flex-col">
    <h1 class="text-5xl text-white mb-4 font-bold">{{ greeting }}</h1>
    <div class="grid w-full  lg:gap-3 gap-4 grid-cols-2 lg:grid-cols-4">
      <div class="relative  lg:h-24 h-48  w-full rounded-lg bg-artist" v-for="music in MusicDetails.music_type"
        ...
        <router-link :to="'/type/' + music.type">
          <div class="absolute w-full lg:h-20 h-34 flex  items-end p-4 opacity-0 hover:opacity-100">
    ...
    <div class="my-3" :v-if="musicStore.recentlyPlayedList">
      <h1 class="text-3xl font-bold text-aliceblue mb-2">Recently Played</h1>
        ...
        <div class="lg:h-64 h-24 bg-artist rounded-lg" v-for="music in musicStore.recentlyPlayedList" :key="music">
          <div class="relateive lg:h-64 h-24 rounded-lg w-full hover:bg-gray-600 flex flex-col justify-between">
            ...
            <div class="flex flex-row pt-4 lg:flex-col">
              <img class="lg:h-40 lg:w-40 h-16 w-16  lg:px-3 ml-3 mx-1" :src="require(`@/assets/images/${music.imgPath}`)" alt="">
              <div>
    ...
    <h1 class="text-4xl mt-3 text-white mb-4 font-bold">You might like this</h1>
    <div class="grid w-full lg:gap-2 gap-4 grid-cols-2 lg:grid-cols-5">
      <div class="relative lg:h-64 h-48 hover:bg-gray-600 bg-artist rounded-lg"
        ...
        <router-link class=" flex flex-col" :to="'/artist/' + music_artist.artist">
          <div class="absolute w-full h-64 flex  items-end p-4 opacity-0 hover:opacity-100">
            <div class="bg-green-500 rounded-full self-end h-8 w-8 flex items-center justify-center">
              <fa class="text-white" icon="play" />
            </div>
          </div>
        </router-link>
        <img class="lg:h-48 h-32 w-32 lg:w-48 mt-4 mx-4 self-center" :src="require(`@/assets/images/${music_artist.imgurl}`)" />
